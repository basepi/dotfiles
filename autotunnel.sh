...
KEYPATH=/Users/cmyers/.ssh/id_rsa_no_passphrase
...
createTunnel() {
    /usr/bin/ssh -f -N -i $KEYPATH -L$SSH_LOCALPORT:$REMOTEHOST:$SSH_REMOTEPORT -L$TUNNEL1_LOCALPORT:$TUNNEL1_REMOTEHOST:$TUNNEL1_REMOTEPORT -L$TUNNEL2_LOCALPORT:$TUNNEL2_REMOTEHOST:$TUNNEL2_REMOTEPORT $REMOTEUSER@$REMOTEHOST
#    if [[ $? -eq 0 ]]; then
